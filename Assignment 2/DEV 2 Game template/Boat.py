...
def draw_boats(boats, offset, screen):

    _width = int(offset / 3)

    # While the boats list is not empty.
    while not boats.IsEmpty:
        screen.blit(pygame.transform.scale(boats.Value.Texture, (_width, _width)), 
                            (_width + boats.Value.Board.Position.X * offset, 
                                _width + boats.Value.Board.Position.Y * offset))
